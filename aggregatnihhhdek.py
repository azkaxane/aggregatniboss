...
# Custom CSS untuk tampilan modern & card KPI
st.markdown("""
<style>
    ...
    .kpi-value { font-size: 24px; color: #212529; font-weight: bold; margin-top: 5px; }
    .recommendation-box {
...
# Input Demand Base via UI Dataframe agar mudah diedit
st.sidebar.subheader("Permintaan (Demand) per Periode")
default_demand = [1200, 1500, 1800, 1400, 1100, 1600]
...
c_regular = st.sidebar.number_input("Biaya Produksi Reguler (/Unit)", value=50000, step=1000)
c_overtime = st.sidebar.number_input("Biaya Overtime (/Unit)", value=75000, step=1000)
c_subcontract = st.sidebar.number_input("Biaya Subcontracting (/Unit)", value=90000, step=1000)
c_inventory = st.sidebar.number_input("Biaya Simpan / Inventory (/Unit/Bulan)", value=10000, step=500)
c_stockout = st.sidebar.number_input("Biaya Stockout / Shortage (/Unit/Bulan)", value=15000, step=500)
c_hiring = st.sidebar.number_input("Biaya Rekrutmen / Hiring (/Pekerja)", value=2000000, step=50000)
c_firing = st.sidebar.number_input("Biaya PHK / Firing (/Pekerja)", value=3500000, step=50000)
...
def calculate_aggregate_planning(strategy, demand_list):
    ...
    # Pra-perhitungan khusus untuk Level Strategy (Menentukan rata-rata kapasitas konstan)
    if strategy == "Level":
    ...
    for t in range(num_periods):
        ...
        if strategy == "Chase":
            ...
            # Mixed strategy menggunakan basis workforce awal/rata-rata, lalu mengandalkan OT & Subcontract
            wf_current = init_workforce
            hiring = 0
            firing = 0
            rt_prod = wf_current * worker_cap

        # 2. Perhitungan Overtime & Subcontracting Berdasarkan Kebutuhan Defisit
        # Defisit dihitung setelah memanfaatkan inventori awal periode
        ...
        if strategy == "Mixed" and deficit > 0:
            ...
            deficit -= ot_prod
            sub_prod = min(max_sub_cap, deficit)
            deficit -= sub_prod
        elif strategy in ["Chase", "Level"] and deficit > 0:
            # Pada pure Chase/Level standard, deviasi diserap oleh inventory/shortage atau adjustment kapasitas murni
            pass
        ...
        cost_labor = wf_current * 3000000  # Nilai proxy gaji bulanan pekerja pokok
        ...
        total_cost = cost_rep + cost_hire + cost_fire + cost_hold + cost_ot + cost_sub + cost_short
        ...
        # Update state untuk iterasi periode berikutnya
        inv_prev = inv_end
        wf_prev = wf_current
...
# ==============================================================================
# 4. EVALUASI METRIK KPI UTAMA & ROBUST PLANNING VIA EXPECTED VALUE
# ==============================================================================
summary_metrics = []
for strat in ["Chase", "Level", "Mixed"]:
    ...
    # Perhitungan Robust Expected Cost
    expected_cost = (c_norm * p_normal) + (c_opt * p_optimistic) + (c_pess * p_pessimistic)
    
    # Ambil metrik performa operasional dari skenario yang sedang dipilih pengguna
    ...
    # Utilisasi Tenaga Kerja & Kapasitas
...
with tab1:
    ...
    # Tampilkan KPI perbandingan menggunakan kolom horizontal
    cols = st.columns(3)
    ...
    # Komparasi Visual Menggunakan Chart Plotly
    c1, c2 = st.columns(2)
    with c1:
    ...
    # Automated Intelligent Recommendation Section
...
with tab2:
    ...
    # Conditional Views Berdasarkan Karakteristik Unik Strategi yang Dipilih
    if selected_strategy == "Chase":
        ...
        mob_df["Internal Content (%)"] = ((mob_df["RT Production"] + mob_df["OT Production"]) / (mob_df["RT Production"] + mob_df["OT Production"] + mob_df["Subcontracting"])) * 100
        st.dataframe(mob_df.style.format(precision=1), use_container_width=True)
        
        # Raw Material Planning Requirement Calculation
    ...
    # Cost Breakdown Per Periode
    st.subheader("💸 Analisis Finansial & Struktur Biaya Berjalan")
    cost_cols = ["Periode", "Production Cost", "Labor Cost", "Hiring Cost", "Firing Cost", "Inventory Holding Cost", "Overtime Cost", "Subcontract Cost", "Shortage Cost", "Total Cost"]
    st.dataframe(df_selected[cost_cols].style.format(precision=0), use_container_width=True)
    
    # Visualisasi Tren Interaktif khusus strategi terpilih
    ...
    with v1:
        ...
        fig_dp.update_layout(title="Perbandingan Tren Permintaan vs Realisasi Pasokan", barmode='group')
        st.plotly_chart(fig_dp, use_container_width=True)
    with v2:
        # Chart 2: Cost Breakdown Stacked Bar
        fig_cb = px.bar(df_selected, x="Periode", y=["Production Cost", "Inventory Holding Cost", "Overtime Cost", "Subcontract Cost", "Shortage Cost"],
                        title="Dinamika Komponen Biaya per Periode", stacked=True)
    ...
    with v4:
        # Chart 4: Overtime vs Subcontracting Comparison
        fig_os = go.Figure()
...
with tab3:
    ...
    # Visualisasi Resiliensi Skenario
