...
# ==============================================================================
# 1. KONFIGURASI HALAMAN & STYLE DASHBOARD (MAROON & WHITE THEME)
# ==============================================================================
st.set_page_config(
...
# Custom CSS untuk tampilan Putih Bersih dengan aksen Maroon
st.markdown("""
<style>
    /* Mengatur latar belakang aplikasi menjadi putih murni */
    .stApp {
        background-color: #ffffff;
    }

    /* Mengatur sidebar agar berwarna putih dengan border halus */
    [data-testid="stSidebar"] {
        background-color: #ffffff !important;
        border-right: 1px solid #f0f0f0;
    }
    
    /* Memastikan teks sidebar berwarna hitam agar kontras tinggi */
    ...
    /* Mengatur Header Tabel menjadi Merah Maroon dengan teks Putih */
    thead tr th {
    ...
    /* Baris tabel dengan warna netral */
    tbody tr td {
        background-color: #ffffff !important;
    ...
    /* KPI Card: Putih dengan bayangan halus dan aksen garis Maroon */
    .kpi-card {
        background-color: #ffffff;
        border-radius: 10px;
        padding: 20px;
    ...
    .kpi-card small { color: #444444; font-weight: 500; }
    
    /* Kotak Rekomendasi: Merah Sangat Muda agar tidak menyilaukan */
    .recommendation-box {
    ...
    .recommendation-box h4 { color: #800000; font-weight: bold; }
    .recommendation-box li, .recommendation-box p { color: #222222; }
...
st.markdown("<p style='color: #800000; font-weight: 600;'>Aplikasi analisis strategi produksi komprehensif dengan pendekatan Robust Planning berbasis skenario.</p>", unsafe_allow_html=True)
st.markdown("---")
...
with tab1:
    ...
    with c1:
        ...
        fig_cost.update_layout(plot_bgcolor='white')
        st.plotly_chart(fig_cost, use_container_width=True)
    with c2:
        fig_sl = px.bar(summary_df, x="Strategi", y="Service Level", 
                        title="Tingkat Layanan Pemenuhan Permintaan (Service Level %)",
                        color_discrete_sequence=['#b03030'], text_auto='.2f', range_y=[0, 105])
        fig_sl.update_layout(plot_bgcolor='white')
...
with tab2:
    ...
    with v1:
        fig_dp = go.Figure()
        fig_dp.add_trace(go.Scatter(x=df_selected["Periode"], y=df_selected["Demand"], name="Demand Real", line=dict(color='#800000', width=2, dash='dash')))
        fig_dp.add_trace(go.Bar(x=df_selected["Periode"], y=df_selected["RT Production"] + df_selected["OT Production"] + df_selected["Subcontracting"], name="Total Produksi", marker_color='#a0a0a0'))
        fig_dp.update_layout(title="Perbandingan Tren Permintaan vs Realisasi Pasokan (12 Bulan)", barmode='group', plot_bgcolor='white')
        st.plotly_chart(fig_dp, use_container_width=True)
    with v2:
        # Menghapus parameter stacked=True yang bermasalah dan menggunakan barmode layout
        fig_cb = px.bar(df_selected, x="Periode", y=["Material Cost", "Production Cost", "Inventory Holding Cost", "Overtime Cost", "Subcontract Cost", "Shortage Cost"],
                        ...
                        color_discrete_sequence=['#800000', '#a52a2a', '#d2691e', '#bc8f8f', '#f5f5f5', '#696969'])
        fig_cb.update_layout(barmode='stack', plot_bgcolor='white')
        st.plotly_chart(fig_cb, use_container_width=True)

    v3, v4 = st.columns(2)
    with v3:
        fig_inv = px.line(df_selected, x="Periode", y="Inventory", title="Fluktuasi Tingkat Inventori Akhir", markers=True, color_discrete_sequence=['#800000'])
        fig_inv.update_layout(plot_bgcolor='white')
        st.plotly_chart(fig_inv, use_container_width=True)
    with v4:
        fig_os = go.Figure()
        fig_os.add_trace(go.Bar(x=df_selected["Periode"], y=df_selected["OT Production"], name="Overtime Vol", marker_color='#800000'))
        fig_os.add_trace(go.Bar(x=df_selected["Periode"], y=df_selected["Subcontracting"], name="Subcontract Vol", marker_color='#cc0000'))
        fig_os.update_layout(title="Alokasi Kapasitas Tambahan: Lembur vs Pihak Ketiga", barmode='stack', plot_bgcolor='white')
...
with tab3:
    ...
    colors_robust = ['#800000', '#a52a2a', '#696969']
    for idx, strat in enumerate(["Chase", "Level", "Mixed"]):
        ...
        fig_robust.add_trace(go.Scatter(
            ...
            line=dict(color=colors_robust[idx])
        ))
    fig_robust.update_layout(title="Analisis Sensitivitas Struktur Biaya Lintas Skenario Permintaan", yaxis_title="Total Biaya (IDR)", plot_bgcolor='white')
