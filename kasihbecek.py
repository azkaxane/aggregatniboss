...
<style>
    ...
    /* Memaksa semua teks menjadi hitam agar terbaca jelas (Anti-Nyaru) */
    ...
    /* --- STYLING TABEL: Putih & Maroon (Tanpa Unsur Dark) --- */
    ...
    /* Body Tabel: Putih bersih dengan teks hitam */
    tbody tr td {
        background-color: #ffffff !important;
        color: #000000 !important;
    ...
    /* KPI Card: Putih dengan aksen garis Maroon */
    .kpi-card {
        background-color: #ffffff;
        border-radius: 12px;
        padding: 20px;
    ...
    /* Kotak Rekomendasi: Latar Belakang Maroon Sangat Muda */
    .recommendation-box {
    ...
    /* Menghilangkan background gelap pada widget input */
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        # Menggunakan warna Maroon untuk bar chart
    ...
    with c2:
        fig_sl = px.bar(summary_df, x="Strategi", y="Service Level", 
                        ...
                        color_discrete_sequence=['#a52a2a'], text_auto='.2f', range_y=[0, 105])
        fig_sl.update_layout(plot_bgcolor='rgba(0,0,0,0)', font=dict(color="black"))
...
with tab2:
    ...
    with v1:
        ...
        fig_dp.update_layout(title="Perbandingan Tren Permintaan vs Realisasi Pasokan", barmode='group', plot_bgcolor='white', font=dict(color="black"))
        st.plotly_chart(fig_dp, use_container_width=True)
    with v2:
        # Perbaikan TypeError: Menggunakan barmode stack di update_layout
        fig_cb = px.bar(df_selected, x="Periode", y=["Material Cost", "Production Cost", "Inventory Holding Cost", "Overtime Cost", "Subcontract Cost", "Shortage Cost"],
                        title="Dinamika Komponen Biaya per Periode", color_discrete_sequence=px.colors.sequential.Reds_r)
    ...
    with v3:
        ...
        fig_inv.update_layout(plot_bgcolor='white', font=dict(color="black"))
    ...
    with v4:
        ...
        fig_os.update_layout(title="Alokasi Kapasitas Tambahan: Lembur vs Pihak Ketiga", barmode='stack', plot_bgcolor='white', font=dict(color="black"))
...
with tab3:
    ...
    st.markdown("Tabel di bawah menyajikan komparasi performa finansial lintas skenario permintaan.")
    ...
    fig_robust.update_layout(title="Analisis Sensitivitas Struktur Biaya Lintas Skenario Permintaan", 
                             yaxis_title="Total Biaya (IDR)", plot_bgcolor='white', font=dict(color="black"))
