...
# Custom CSS untuk warna Merah Maroon (#800000) dan Putih (#FFFFFF)
st.markdown("""
<style>
    ...
    /* Mengatur warna teks di sidebar agar kontras dengan putih */
    [data-testid="stSidebar"] .stMarkdown, [data-testid="stSidebar"] label, [data-testid="stSidebar"] p {
        color: #333333 !important;
    }
    
    /* Judul di sidebar menjadi maroon */
    [data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 {
        color: #800000 !important;
    }

    /* Kepala Tabel (Header) berwarna Merah Maroon */
    thead tr th {
    ...
    /* KPI Card: Putih dengan border Maroon */
    .kpi-card {
        background-color: #ffffff;
        border-radius: 8px;
        padding: 20px;
    ...
    /* Kotak Rekomendasi: Latar merah sangat muda agar teks hitam terbaca jelas */
    .recommendation-box {
    ...
    .recommendation-box li, .recommendation-box p { color: #111111; }
</style>
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        # Menggunakan warna Maroon untuk grafik batang biaya
    ...
    with c2:
        # Menggunakan warna Maroon Muda untuk grafik service level
    ...
    <div class="recommendation-box">
        ...
        <ul>
            <li><b>Efisiensi Biaya Terbaik (Robust):</b> Strategi <b>{best_cost_strat}</b> memberikan nilai ekonomis paling tangguh (Expected Cost terendah).</li>
            <li><b>Keterandalan Layanan (Service Level):</b> Strategi <b>{best_sl_strat}</b> paling optimal meminimalisir risiko stockout.</li>
            <li><b>Efisiensi Fasilitas (Utilisasi Kapasitas):</b> Strategi <b>{best_util_strat}</b> mencatatkan penggunaan infrastruktur paling produktif.</li>
        </ul>
        <p><b>Rekomendasi Final:</b> Disarankan menggunakan pendekatan <b>{best_cost_strat} Strategy</b> untuk mengamankan struktur finansial perusahaan.</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab2:
    ...
    st.subheader(f"📋 Master Table Aggregate Production Planning: {selected_strategy}")
    ...
    if selected_strategy == "Chase":
        st.subheader("👨‍🏭 Workforce & Capacity Adjustment Sheet")
        ...
        st.subheader("📦 Inventory Buffer & Capacity Efficiency Sheet")
        ...
        st.subheader("🔄 Sourcing Optimization & Make-or-Buy Analysis")
    ...
    st.subheader("💸 Analisis Finansial & Struktur Biaya")
    ...
    with v1:
        ...
        fig_dp.update_layout(title="Perbandingan Tren Permintaan vs Pasokan", barmode='group')
        st.plotly_chart(fig_dp, use_container_width=True)
    with v2:
        # Perbaikan Error Stacked menggunakan barmode='stack'
...
with tab3:
    ...
    st.markdown("Komparasi performa finansial lintas skenario.")
    ...
    # Menggunakan palet warna Maroon yang berbeda untuk tiap garis
    colors = ['#800000', '#c03030', '#600000']
    for i, strat in enumerate(["Chase", "Level", "Mixed"]):
        ...
        fig_robust.add_trace(go.Scatter(
            ...
            mode='lines+markers', name=f"Profil {strat}", line=dict(color=colors[i])
        ))
    fig_robust.update_layout(title="Analisis Sensitivitas Biaya", yaxis_title="Total Biaya (IDR)")
